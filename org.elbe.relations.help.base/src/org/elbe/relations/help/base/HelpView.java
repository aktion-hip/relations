...
public class HelpView {
    ...
    @Inject
    public HelpView(final Composite inParent, final IExtensionRegistry inRegistry) {

        final IConfigurationElement[] lHelps = inRegistry.getConfigurationElementsFor(Constants.HELP_PLUGIN_ID);
        for (final IConfigurationElement lHelp : lHelps) {
            if (HelpManager.TOC_MARKER.equals(lHelp.getName())) {
                final HelpManager lManager = getHelpManager(lHelp.getNamespaceIdentifier());
                lManager.process(lHelp);
            }
        }

        try {
            for (final HelpManager lManager : this.helpManagers.values()) {
            ...
            new HelpNavigationJS(lBrowser, "tocRenderer", this.helpManagers.values());
            lBrowser.addProgressListener(new ProgressListener() {

                @Override
                public void completed(final ProgressEvent inEvent) {
                    final String lUrl = (String) lBrowser.evaluate("return parent.ContentFrame.location.href;");
                    if (lUrl != null) {
                        lBrowser.execute(getBreadcrumb(getBreadcrumbItems(lUrl, HelpView.this.helpManagers)));
                    }
                }
    ...
    private String getBreadcrumbItems(final String inUrl, final Map<String, HelpManager> inHelpManagers) {

        HelpManager lManager = null;
        String lNamespace = "";
        for (final Entry<String, HelpManager> lEntry : inHelpManagers.entrySet()) {
            final String lNamespacePart = Constants.PATH_DELIM + lEntry.getKey() + Constants.PATH_DELIM;
        ...
        return lManager == null ? "" : lManager.getBreadcrumbs(lNamespace, inUrl);
    }

    private String getBreadcrumb(final String inBreadcrumbItems) {
        final StringBuilder out = new StringBuilder("var frame = parent.ContentFrame.document;");
        ...
        out.append("breadcrumb.innerHTML = '").append(inBreadcrumbItems).append("';");
    ...
    private HelpManager getHelpManager(final String inHelpIdentifier) {
        HelpManager out = this.helpManagers.get(inHelpIdentifier);
        if (out == null) {
            out = new HelpManager(inHelpIdentifier, this.log);
            this.helpManagers.put(inHelpIdentifier, out);
        }
        return out;
    }
    ...
    /** JavaScript callback class to insert the html for the navigation into the navigation frame. */
    class HelpNavigationJS extends BrowserFunction {
        ...
        public Object function(final Object[] inArguments) {
            ...
            try {
                for (final HelpManager lHelp : this.helpTocs) {
                    out.append(Constants.LI_START).append(lHelp.renderToc()).append(Constants.LI_END);
                }
            } catch (final IOException exc) {
                HelpView.this.log.error(exc, exc.getMessage());
            }
            return new String(out);
        }
    }
