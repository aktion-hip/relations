...
public class ExtractorUtil {
     ...
     * @throws IOException
     */
    public static long getNumericalValue(final FileDataSource inDataSource, final int inSize, final boolean inGetUpper) throws IOException {
        final byte[] lBytes = inDataSource.getData(inSize);
     ...
     */
    public static long getNumericalValue(final byte[] inBytes, final boolean inGetUpper) {
        long out = -1;
        if (!inGetUpper) {
     ...
     * @throws IOException
     */
    public static String getFixedStringValue(final FileDataSource inSource, final int inLength) throws IOException {
     ...
     */
    public static boolean checkFileHeader(final File inFile, final String inTest) {
        final int lLength = (inTest.length() + 1) / 3;
        final int[] lValue = new int[lLength];
        final int[] lMask = new int[lLength];

        final StringTokenizer lTokenizer = new StringTokenizer(inTest, " "); //$NON-NLS-1$
        String lToken = null;
    ...
    private static boolean checkFileHeader(final File inFile, final int[] inTest, final int[] inMask) {
        final int lLength = inTest.length;
        final byte[] lBuffer = new byte[lLength];
     ...
     * @return String
     */
    public static String toHexFilter(final String inAscii) {
        final StringBuilder out = new StringBuilder();

        final char[] lChars = inAscii.toCharArray();
        for(int i=0; i<lChars.length; i++) {
            out.append(Integer.toHexString(lChars[i]));
