...
public class COSString extends COSBase {
	...
	public COSString(String inValue) {
		try {
			...
			final char[] lChars = inValue.toCharArray();
			...
			if (lUnicode16) {
				final byte[] lData = inValue.getBytes("UTF-16BE"); //$NON-NLS-1$
				out = new ByteArrayOutputStream(lData.length + 2);
				out.write(0xFE);
				out.write(0xFF);
				out.write(lData);
			} else {
				final byte[] lData = inValue.getBytes("ISO-8859-1"); //$NON-NLS-1$
	 ...
	 * @param inHex
	 *            A hex string.
	 * @return A cos string with the hex characters converted to their actual
	 *         bytes.
	 * @throws IOException
	 *             If there is an error with the hex string.
	 */
	public static COSString createFromHexString(String inHex) throws IOException {
		final COSString outValue = new COSString();
		final StringBuilder lHexBuffer = new StringBuilder(inHex.trim());
		// if odd number then the last hex digit is assumed to be 0
		if (lHexBuffer.length() % 2 != 0) {
			lHexBuffer.append("0"); //$NON-NLS-1$
		}
		for (int i = 0; i < lHexBuffer.length();) {
			final String lHexChars = "" + lHexBuffer.charAt(i++) + lHexBuffer.charAt(i++); //$NON-NLS-1$
			try {
