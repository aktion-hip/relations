...
public class BookmarkHandler {
    private final static String SUCCESS_MSG = RelationsMessages.getString("BookmarkHandler.msg.success"); //$NON-NLS-1$
    ...
    @Execute
    void createBookmark(final IEclipseContext context, final RelationsStatusLineManager statusLine) {
        final ItemAdapter selected = this.browserManager.getSelectedModel();
        if (selected == null) {
            return;
        }
        ...
        try {
            ...
            this.bookmarksController.addItem(new RetrievedItemWithIcon(selected.getUniqueID(), title));
            statusLine.showStatusLineMessage(String.format(SUCCESS_MSG, title));
        }
