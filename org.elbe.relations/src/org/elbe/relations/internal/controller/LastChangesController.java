...
public class LastChangesController {
	...
	public Collection<AlternativeModel> getLastChangedItems()
		...
		final List<AlternativeModel> outItems = new ArrayList<AlternativeModel>();
		...
		outItems.addAll(
		        getPersons(lastChangesType.getFieldName(), lMaxEntries));
		...
		final List<AlternativeModel> outResized = new ArrayList<AlternativeModel>(
		        lMaxEntries);
		int i = 0;
		for (final Iterator<AlternativeModel> lItems = outItems
		        .iterator(); lItems.hasNext() && i++ < lMaxEntries;) {
	...
	void setViewState(
		...
		if (inLastChangeState == null || inLastChangeState.isEmpty()) {
			...
			lastChangesType = Boolean.parseBoolean(inLastChangeState)
	...
	private Collection<AlternativeModel> getTerms(final String inOrderKey,
		...
		final QueryResult lResult = BOMHelper.getCollectableTermHome()
		        .select(createOrderObject(inOrderKey));
		return lResult.load(new ModelFactory(IItem.TERM, "TermID"), //$NON-NLS-1$
		        inMaxEntries);
	}

	private Collection<AlternativeModel> getTexts(final String inOrderKey,
	        final int inMaxEntries) throws VException, SQLException {
		final QueryResult lResult = BOMHelper.getCollectableTextHome()
		        .select(createOrderObject(inOrderKey));
		return lResult.load(new ModelFactory(IItem.TEXT, "TextID"), //$NON-NLS-1$
		        inMaxEntries);
	}
	...
	private Collection<AlternativeModel> getPersons(final String inOrderKey,
		...
		final QueryResult lResult = BOMHelper.getCollectablePersonHome()
		        .select(createOrderObject(inOrderKey));
		return lResult.load(new PersonModelFactory(IItem.PERSON, "PersonID"), //$NON-NLS-1$
		        inMaxEntries);
	}
	...
	private static class ModelFactory implements AlternativeModelFactory {
		...
		public AlternativeModel createModel(final ResultSet inResultSet)
			...
			return new RetrievedChronologicalItem(
			        new UniqueID(itemType, inResultSet.getLong(fieldID)),
	...
	private static class PersonModelFactory extends ModelFactory {
		public PersonModelFactory(final int inItemType,
		        final String inFieldID) {
	...
	private static class SortOnMutationDate
	        implements Comparator<AlternativeModel>, Serializable {
		@Override
	...
	@SuppressWarnings("serial")
	private static class SortOnCreationDate
	        implements Comparator<AlternativeModel>, Serializable {
		@Override
		public int compare(final AlternativeModel inItem1,
		        final AlternativeModel inItem2) {
