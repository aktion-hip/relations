...
/** Object intended to be stored in the eclipse context providing the data to access the database.<br />
 * An instance of this class is in the context as <code>DBSettings.class</code>.
 *
 * @author Luthiger */
@Creatable
public class DBSettings implements IDBSettings {
    protected static final MessageFormat CLIENT_URL_PATTERN = new MessageFormat("jdbc:{0}://{1}/{2}"); //$NON-NLS-1$
    protected static final String EMBEDDED = RelationsMessages.getString("DBHandler.embedded"); //$NON-NLS-1$
     ...
     * @param dbController */
    @Inject
    public DBSettings(@Preference(nodePath = RelationsConstants.PREFERENCE_NODE) final IEclipsePreferences preferences,
            final IDBController dbController) {
        this.dbController = dbController;
        this.dbConnectionConfig = dbController.getConfiguration(
                preferences.get(RelationsConstants.KEY_DB_PLUGIN_ID, RelationsConstants.DFT_DBCONFIG_PLUGIN_ID));
        this.host = preferences.get(RelationsConstants.KEY_DB_HOST, ""); //$NON-NLS-1$
        this.catalog = preferences.get(RelationsConstants.KEY_DB_CATALOG, RelationsConstants.DFT_DB_EMBEDDED);
    ...
    /** @return {@link IDBConnectionConfig} the used DB's connection configuration, may be <code>null</code> */
    @Override
    ...
    public String getDBName() {
        ...
        return CLIENT_URL_PATTERN
                .format(new Object[] { getDBConnectionConfig().getSubprotocol(), getHost(), getCatalog() });
    }
