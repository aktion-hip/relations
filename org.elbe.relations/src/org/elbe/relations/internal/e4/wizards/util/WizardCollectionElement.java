...
public class WizardCollectionElement extends AdaptableList implements IWizardDescriptor, IWizardCategory, IAdaptable {
    ...
    public WizardCollectionElement(final IConfigurationElement inElement,
        ...
        this.configElement = inElement;
        this.id = this.configElement.getAttribute(IWorkbenchRegistryConstants.ATT_ID);
    ...
    public IPath getPath() {
        ...
        return this.parent.getPath().append(getId());
    }
     ...
     */
    @Override
    public IWizardDescriptor findWizard(final String inSearchId) {
    ...
    public WorkbenchWizardElement findWizard(final String inSearchId,
        ...
        for (final Iterator lChildren = this.children.iterator(); lChildren
                .hasNext();) {
            final WizardCollectionElement lChild = (WizardCollectionElement) lChildren
                    .next();
            final WorkbenchWizardElement lResult = lChild.findWizard(inSearchId,
                    true);
            if (lResult != null) {
                return lResult;
            }
    ...
    public IWizardDescriptor[] getWizards() {
        return (IWizardDescriptor[]) this.wizards
     ...
     * (non-Javadoc)
     *
     * @see org.eclipse.ui.model.AdaptableList#remove(org.eclipse.core.runtime.
     * IAdaptable )
     */
    @Override
    public void remove(final IAdaptable inAdaptable) {
        if (inAdaptable instanceof WorkbenchWizardElement) {
            this.wizards.remove(inAdaptable);
        } else {
    ...
    public String getPluginId() {
        return this.configElement == null ? this.pluginId
                : this.configElement.getNamespaceIdentifier();
    }

    @Override
    public String getLabel(final Object inObject) {
        return this.configElement == null ? this.name
                : this.configElement
    ...
    public WizardCollectionElement[] getCategories() {
        return (WizardCollectionElement[]) getTypedChildren(
    ...
    public String getDescription() {
        return Util.getDescription(this.configElement);
    }
    ...
    public WizardCollectionElement findChildCollection(final IPath inPath) {
        ...
        for (int i = 0; i < lChildren.length; ++i) {
            ...
            if (lCurrentCategory.getId().equals(lSearchString)) {
                ...
                return lCurrentCategory
                        .findChildCollection(inPath.removeFirstSegments(1));
            }
        }
        return null;
    }
    ...
    public WorkbenchWizardElement[] getWorkbenchWizardElements() {
        return getWorkbenchWizardElementsExpression(
                (WorkbenchWizardElement[]) this.wizards
