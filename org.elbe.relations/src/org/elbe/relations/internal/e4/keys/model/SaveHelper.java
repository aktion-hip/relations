...
public class SaveHelper {
    ...
    @Inject
    public SaveHelper(final MApplication application, final ECommandService commandService, final Logger log,
            final BindingManager manager) {
     ...
     */
    public Collection<BindingRestoreHelper> setDefault(final Set<String> contexts) {
        final Collection<BindingRestoreHelper> outFactories = new ArrayList<SaveHelper.BindingRestoreHelper>();
        for (final String context : contexts) {
            outFactories
            .add(setDefault(getMTable(context), this.application, context));
        }
        return outFactories;
    }
    ...
    private BindingRestoreHelper setDefault(final MBindingTable table,
        ...
        final BindingRestoreHelper outBindingRestoreFactory = new BindingRestoreHelper(
                contextId, this.commandService, this.log);
     ...
     */
    public MKeyBinding addBinding(final Binding newBinding) {
        final MBindingTable table = getMTable(newBinding.getContextId());
        return createOrUpdateMKeyBinding(this.application, table, newBinding);
    }

    private MKeyBinding createOrUpdateMKeyBinding(final MApplication inApplication,
            final MBindingTable table, final Binding newBinding) {
    ...
    private MKeyBinding createDeleted(final MCommand command,
            final String triggerSequence) {
    ...
    private String getOrigSequence(final MKeyBinding toReplace) {
    ...
    private boolean isSameBinding(final MKeyBinding existingBinding, final MCommand cmd,
    ...
    private MBindingTable getMTable(final String contextId) {
        for (final MBindingTable outBindingTable : this.application
                .getBindingTables()) {
        ...
        this.application.getBindingTables().add(outTable);
        return outTable;
    }

    private MBindingContext getBindingContext(final String contextId) {
        // cache
        MBindingContext lResult = this.bindingContexts.get(contextId);
        if (lResult == null) {
            // search
            lResult = searchContexts(contextId, this.application.getRootContext());
            if (lResult == null) {
                // create
                ...
                this.application.getRootContext().add(lResult);
            }
            if (lResult != null) {
                this.bindingContexts.put(contextId, lResult);
            }
        }
        return lResult;
    }
    ...
    public Scheme getScheme(final String inSchemeId) {
        return this.manager.getScheme(inSchemeId);
    }
    ...
    public static class BindingRestoreHelper {
        ...
        BindingRestoreHelper(final String inContextId,
            ...
            this.bindings = new ArrayList<MKeyBinding>();
            this.contextId = inContextId;
            this.commandService = inCommandService;
            this.log = inLog;
        }

        void registerRestoredBinding(final MKeyBinding inBinding) {
            this.bindings.add(inBinding);
        }
