...
public class RelationsKeysPreferencePage extends AbstractPreferencePage {
	...
	private void fill() {
		...
		schemeCombo.setSelection(new StructuredSelection(keyController
	...
	private Control createButtonBar(final Composite inParent) {
		...
		lGridData = new GridData();
		lWidthHint = convertHorizontalDLUsToPixels(IDialogConstants.BUTTON_WIDTH);
		lFiltersButton.setText(NewKeysPreferenceMessages.FiltersButton_Text);
		lGridData.widthHint = Math.max(lWidthHint,
				lFiltersButton.computeSize(SWT.DEFAULT, SWT.DEFAULT, true).x) + 5;
		...
		lFiltersButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(final SelectionEvent inEvent) {
				...
				lDialog.setFilterUncategorized(filteredTree
				...
				if (lDialog.open() == Window.OK) {
					...
					filteredTree.filterCategories(lDialog
		...
		lGridData.widthHint = Math.max(lWidthHint,
				lExportButton.computeSize(SWT.DEFAULT, SWT.DEFAULT, true).x) + 5;
		...
		lExportButton.addSelectionListener(new SelectionListener() {
			...
			@Override
			public void widgetSelected(final SelectionEvent inEvent) {
				keyController.exportCSV(((Button) inEvent.getSource())
						.getShell());
			}
	...
	private void createDataControls(final Composite inParent) {
		...
		final Label lCommandDescriptionlabel = new Label(lLeftDataArea,
				SWT.LEAD);
		lCommandDescriptionlabel
				.setText(NewKeysPreferenceMessages.CommandDescriptionLabel_Text);
		lGridData = new GridData();
		lGridData.verticalAlignment = SWT.BEGINNING;
		lCommandDescriptionlabel.setLayoutData(lGridData);

		descriptionText = new Text(lLeftDataArea, SWT.MULTI | SWT.WRAP
		...
		final Button lAddKeyButton = new Button(lLeftDataArea, SWT.LEFT
				| SWT.ARROW);
		lAddKeyButton
		...
		lAddKeyButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent inSelectionEvent) {
				...
				lAddKeyMenu.setLocation(lButtonLocation.x, lButtonLocation.y
						+ lButtonSize.y);
		...
		final IPropertyChangeListener lWhenListener = new IPropertyChangeListener() {
			...
			public void propertyChange(final PropertyChangeEvent inEvent) {
				if (inEvent.getSource() == keyController.getContextModel()
					...
					final StructuredSelection structuredSelection = newVal == null ? null
		...
		conflictViewer = new TableViewer(lRightDataArea, SWT.SINGLE
				| SWT.V_SCROLL | SWT.BORDER | SWT.FULL_SELECTION);
		final Table lTable = conflictViewer.getTable();
		lTable.setHeaderVisible(true);
		final TableColumn lBindingNameColumn = new TableColumn(lTable, SWT.LEAD);
		lBindingNameColumn
		...
		conflictViewer.setLabelProvider(new BindingElementLabelProvider() {
			...
			public String getColumnText(final Object inObj, final int inIndex) {
				...
				return lElement.getContext() == null ? "" : lElement
						.getContext().getName();
			}
		});
		conflictViewer
			...
			public void propertyChange(final PropertyChangeEvent inEvent) {
				if (inEvent.getSource() == keyController.getConflictModel()
					...
					if (keyController.getConflictModel().getConflicts() != null) {
						...
						final StructuredSelection structuredSelection = lNewVal == null ? null
								: new StructuredSelection(lNewVal);
						conflictViewer.setSelection(structuredSelection, true);
					}
				} else if (ConflictModel.PROP_CONFLICTS.equals(inEvent
						.getProperty())) {
					conflictViewer.setInput(inEvent.getNewValue());
				} else if (ConflictModel.PROP_CONFLICTS_ADD.equals(inEvent
						.getProperty())) {
					conflictViewer.add(inEvent.getNewValue());
				} else if (ConflictModel.PROP_CONFLICTS_REMOVE.equals(inEvent
						.getProperty())) {
	...
	private Control createTreeControls(final Composite inParent) {
		...
		lGridData.widthHint = Math
				.max(lWidthHint, lAddBindingButton.computeSize(SWT.DEFAULT,
						SWT.DEFAULT, true).x) + 5;
	...
	private void createTree(final Composite inParent) {
		...
		filteredTree = new CategoryFilterTree(inParent, SWT.SINGLE
		...
		lTriggerSequenceColumn.addSelectionListener(new ResortColumn(
				lComparator, lTriggerSequenceColumn, lViewer,
				KEY_SEQUENCE_COLUMN));
		...
		final IPropertyChangeListener lTreeUpdateListener = new IPropertyChangeListener() {
			...
			public void propertyChange(final PropertyChangeEvent inEvent) {
				...
				} else if (BindingElement.PROP_CONFLICT.equals(inEvent
						.getProperty())) {
					lViewer.update(inEvent.getSource(), null);
				} else if (BindingModel.PROP_BINDINGS.equals(inEvent
						.getProperty())) {
					lViewer.refresh();
				} else if (BindingModel.PROP_BINDING_ADD.equals(inEvent
						.getProperty())) {
					lViewer.add(keyController.getBindingModel(),
							inEvent.getNewValue());
				} else if (BindingModel.PROP_BINDING_REMOVE.equals(inEvent
						.getProperty())) {
					lViewer.remove(inEvent.getNewValue());
				} else if (BindingModel.PROP_BINDING_FILTER.equals(inEvent
						.getProperty())) {
					lViewer.refresh();
				}
			}
	...
	private void createSchemeControls(final Composite inParent) {
		...
		schemeCombo
				.addSelectionChangedListener(new ISelectionChangedListener() {
					...
					public void selectionChanged(
							final SelectionChangedEvent inEvent) {
						BusyIndicator.showWhile(filteredTree.getViewer()
	...
	private class BindingElementLabelProvider extends LabelProvider implements
		...
		public String getText(final Object inElement) {
			final String lText = getColumnText(inElement, 0);
			if (lText == null) {
				super.getText(inElement);
			}
			final StringBuilder outText = new StringBuilder(lText);
		...
		public String getColumnText(final Object inElement, final int inIndex) {
			final BindingElement lBindingElement = ((BindingElement) inElement);
			switch (inIndex) {
			case COMMAND_NAME_COLUMN: // name
				return lBindingElement.getName();
			case KEY_SEQUENCE_COLUMN: // keys
				final TriggerSequence lSequence = lBindingElement.getTrigger();
				return lSequence == null ? Util.ZERO_LENGTH_STRING : lSequence
						.format();
			case CONTEXT_COLUMN: // when
				final ModelElement lContext = lBindingElement.getContext();
				return lContext == null ? Util.ZERO_LENGTH_STRING : lContext
						.getName();
			case CATEGORY_COLUMN: // category
				return lBindingElement.getCategory();
			case USER_DELTA_COLUMN: // user
				if (lBindingElement.getUserDelta().intValue() == Binding.USER) {
					if (lBindingElement.getConflict().equals(Boolean.TRUE)) {
						return "CU"; //$NON-NLS-1$
					}
					return " U"; //$NON-NLS-1$
				}
		...
		public Image getColumnImage(final Object inElement, final int inIndex) {
			final BindingElement lBinding = (BindingElement) inElement;
			switch (inIndex) {
			case COMMAND_NAME_COLUMN:
				...
				//					final String lMessage = "Problem retrieving image for a command '" //$NON-NLS-1$
				// + lCommandId + '\'';
