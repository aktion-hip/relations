...
public class CheckDirtyService {
    ...
    public CheckDirtyService(final AbstractEditForm inEditForm) {
        this.toCheck = new ArrayList<ICheckDirtyElement>();
        this.editForm = inEditForm;
    }

    public void register(final RequiredText inText) {
        this.toCheck.add(new CheckDirtyRequiredText(inText, this));
    }

    public void register(final StyledTextComponent inText) {
        this.toCheck.add(new CheckDirtyStyledTextComponent(inText, this));
    }

    public void register(final StyledText inText) {
        this.toCheck.add(new CheckDirtyStyledText(inText, this));
    }

    public void register(final Text inText) {
        this.toCheck.add(new CheckDirtyText(inText, this));
    }

    public void register(final Combo inCombo) {
        this.toCheck.add(new CheckDirtyCombo(inCombo, this));
    }

    public void register(final Button inButton) {
        this.toCheck.add(new CheckDirtyButton(inButton, this));
    }

    public void freeze() {
        for (final ICheckDirtyElement lElement : this.toCheck) {
            lElement.freeze();
        }
        this.isDirty = false;
    }
    ...
    public void notifyDirtySwitch(final boolean inIsDirty) {
        if (this.isDirty ^ inIsDirty) {
            ...
            if (this.isDirty ^ lIsDirty) {
                ...
                this.editForm.notifyDirtySwitch(this.isDirty);
            }
        }
    }
    ...
    protected boolean getDirty() {
        ...
        for (final ICheckDirtyElement element : this.toCheck) {
            isDirty |= element.getDirty();
        }
        return isDirty;
    }
    ...
    private abstract class AbstractCheckDirty {
        ...
        protected void handleEvent() {
            ...
            if (this.isDirty ^ lIsDirty) {
                ...
                this.isDirty = lIsDirty;
                this.service.notifyDirtySwitch(lIsDirty);
            }
        }
    ...
    ICheckDirtyElement {
        ...
        public CheckDirtyRequiredText(final RequiredText inText,
            ...
            super(inService);
            this.text = inText;
            this.text.addModifyListener(createModifyListener());
        }
        ...
        protected boolean checkDirty() {
            return !this.originalValue.equals(this.text.getText());
        }
    ...
    implements ICheckDirtyElement {
        ...
        public void freeze() {
            this.originalValue = this.text.getText();
            this.originalStyle = this.text.getStyleRanges();
            this.bulletsState = this.text.getBulletsState();
            this.initialized = true;
            this.isDirty = false;
        }
        ...
        @Override
        protected boolean checkDirty() {
            return !this.originalValue.equals(this.text.getText()) || styleChanged()
                    || bulletsChanged();
        }

        private boolean bulletsChanged() {
            if (this.bulletsState == null) {
                return false;
            }
            return !this.bulletsState.equals(this.text.getBulletsState());
        }

        private boolean styleChanged() {
            final StyleRange[] lRanges = this.text.getStyleRanges();
            if (this.originalStyle.length != lRanges.length) {
                return true;
            }
            for (int i = 0; i < lRanges.length; i++) {
                if (!lRanges[i].equals(this.originalStyle[i])) {
                    return true;
                }
            }
            return false;
        ...
        public void undo() {
            this.text.setText(this.originalValue);
            this.text.setStyleRanges(this.originalStyle);
            this.bulletsState.undo((StyledText) this.text.getControl());
        }
    ...
    ICheckDirtyElement {
        ...
        public CheckDirtyCombo(final Combo inCombo,
            ...
            super(inService);
            this.combo = inCombo;
            this.combo.addModifyListener(createModifyListener());
        }

        @Override
        public void freeze() {
            this.originalValue = this.combo.getSelectionIndex();
            this.initialized = true;
            this.isDirty = false;
        }
        ...
        @Override
        protected boolean checkDirty() {
            return this.originalValue != this.combo.getSelectionIndex();
        }

        @Override
        public void undo() {
            this.combo.select(this.originalValue);
        }
    ...
    ICheckDirtyElement {
        ...
        public CheckDirtyButton(final Button inButton,
            ...
            this.button.addFocusListener(createFocusListener());
        }

        @Override
        public void freeze() {
            this.originalValue = this.button.getSelection();
            this.initialized = true;
            this.isDirty = false;
        }
        ...
        @Override
        protected boolean checkDirty() {
            return this.originalValue ^ this.button.getSelection();
        }

        @Override
        public void undo() {
            this.button.setSelection(this.originalValue);
        }
