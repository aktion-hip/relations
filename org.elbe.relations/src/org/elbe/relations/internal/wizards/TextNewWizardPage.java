...
IItemWizardPage {
    ...
    public void createControl(final Composite inParent) {
        ...
        this.form = FormText.createFormText(inParent, false, this.context);
    ...
    public void save() throws BOMException {
        ...
        final AbstractText lText = lHome.newText(this.form.getTextTitle(),
                this.form.getTextText(), this.form.getAuthorName(),
                this.form.getCoAuthorName(), this.form.getSubTitle(), this.form.getYear(),
                this.form.getJournal(), this.form.getPages(),
                Integer.valueOf(this.form.getArticleVolume()),
                Integer.valueOf(this.form.getArticleNumber()), this.form.getPublisher(),
                this.form.getLocation(), Integer.valueOf(this.form.getTextType()));
        getRelationsSaveHelper().saveWith(lText);
        this.data.loadNew((LightWeightText) lText.getLightWeight());
        try {
            this.eventBroker.post(
                    RelationsConstants.TOPIC_TO_BROWSER_MANAGER_SET_MODEL,
                    new TextWithIcon(lText, this.context));
        }
