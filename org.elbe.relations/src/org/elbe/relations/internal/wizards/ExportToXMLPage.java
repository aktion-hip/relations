...
public class ExportToXMLPage extends BackupEmbeddedPage {
	...
	protected void setFilterForDialog(final FileDialog inDialog) {
		...
		inDialog.setFilterNames(new String[] {
				RelationsMessages.getString("ExportToXMLPage.filter.plain"), RelationsMessages.getString("ExportToXMLPage.filter.zipped") }); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected String postProcessFileName(final String inFileName) {
		if (inFileName.endsWith(".zip") || inFileName.endsWith(".xml"))return inFileName; //$NON-NLS-1$ //$NON-NLS-2$
