...
public abstract class AbstractRelationsWizardPage extends WizardPage {
    ...
    AbstractRelationsWizardPage(final String pageName) {
        super(pageName);

        // empty implementation of the IRelationsSaveHelper
        this.relationsSave = new IRelationsSaveHelper() {
            @Override
     ...
     *
     * @param form AbstractEditForm */
    protected void configureForm(final AbstractEditForm form) {
        form.initialize();
        setControl(form.getControl());
    }

    /** Makes this wizard page updating on events sent by the form. The messages and button statuses are modified
     * according to the statuses reported.
     *
     * @param status {@link IStatus} */
    @Inject
    @Optional
    public void updateHandler(
            @UIEventTopic(RelationsConstants.TOPIC_WIZARD_PAGE_STATUS) final IStatus status) {
     ...
     *
     * @param fieldContent Object the result of model.get(KEY)
     * @return String maybe an empty string if the object provided is null */
    protected String getField(final Object fieldContent) {
    ...
    /** Functionality implementing the <code>IRelationsSaveHelper</code> is processed after the new item is saved and,
     * thus, initialized by a unique item ID. This item ID is needed to save the item's relations.
     *
     * @param helper IRelationsSaveHelper */
    public void setRelationsSaveHelper(final IRelationsSaveHelper helper) {
        this.relationsSave = helper;
    }

    protected IRelationsSaveHelper getRelationsSaveHelper() {
        return this.relationsSave;
    }
