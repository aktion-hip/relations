...
public class WizardsRegistryReader {
	...
	private void finishPrimary() {
		if (deferPrimary != null) {
			...
			for (final Iterator<String> lIds = deferPrimary.iterator(); lIds
				...
				final WorkbenchWizardElement lElement = wizardElements == null ? null
						: wizardElements.findWizard(lId, true);
				if (lElement != null) {
	...
	private void pruneEmptyCategories(final WizardCollectionElement inParent) {
		...
		for (int i = 0; i < lChildren.length; i++) {
			...
			final boolean lShouldPrune = lChild.getId().equals(
	...
	private void finishCategory(final Category inCategory) {
		...
		if (lParent != null) {
			createCollectionElement(lParent,
					(IConfigurationElement) Util.getAdapter(inCategory,
	...
	private boolean readElement(final IConfigurationElement inElement) {
		if (inElement.getName()
			...
			return true;
		} else if (inElement.getName().equals(
				IWorkbenchRegistryConstants.TAG_PRIMARYWIZARD)) {
			if (deferPrimary == null) {
				deferPrimary = new HashSet<String>();
			}
			deferPrimary.add(inElement
					.getAttribute(IWorkbenchRegistryConstants.ATT_ID));
			return true;
		} else {
			if (!inElement.getName().equals(
					IWorkbenchRegistryConstants.TAG_WIZARD)) {
				return false;
			}
			final WorkbenchWizardElement lWizard = createWizardElement(inElement);
			if (lWizard != null) {
	...
	private WorkbenchWizardElement createWizardElement(
		...
		if (inElement.getAttribute(IWorkbenchRegistryConstants.ATT_NAME) == null) {
			logMissingAttribute(inElement, IWorkbenchRegistryConstants.ATT_NAME);
			return null;
		}

		if (getClassValue(inElement, IWorkbenchRegistryConstants.ATT_CLASS) == null) {
	...
	public static IExtension[] orderExtensions(final IExtension[] inExtensions) {
	...
	private void logError(final IConfigurationElement inElement,
		...
		lMessage.append("Plugin ").append(lExtension.getNamespaceIdentifier()).append(", extension ").append(lExtension.getExtensionPointUniqueIdentifier());//$NON-NLS-2$//$NON-NLS-1$
