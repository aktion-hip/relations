...
public class PrintOutWizardPage extends ExportWizardPage {
	...
	private static final String[] radioScopeLabels = new String[] {
			RelationsMessages.getString("PrintOutWizardPage.scope.1"), RelationsMessages.getString("PrintOutWizardPage.scope.2"), RelationsMessages.getString("PrintOutWizardPage.scope.3") }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	private final Collection<Control> controls1 = new Vector<Control>();
	private final Collection<Control> controls2 = new Vector<Control>();
	...
	public PrintOutWizardPage() {
		...
		setTitle(RelationsMessages.getString("PrintOutWizardPage.dialog.title")); //$NON-NLS-1$
	...
	public void createControl(final Composite inParent) {
		...
		printFileName = createLabelCombo(
				lComposite,
				RelationsMessages.getString("PrintOutWizardPage.lbl.file"), SWT.DROP_DOWN); //$NON-NLS-1$
	...
	private Collection<Control> createAppendOrNew(final Composite inParent) {
		...
		return createRadios(lSelectionPane, radioProcessIds,
				radioProcessLabels, PROCESS_APPEND,
				new ProcessSelectionListener());
	}

	private Combo createComboPrintType(final Composite inParent) {
		final Combo outCombo = createLabelCombo(
				inParent,
				RelationsMessages.getString("PrintOutWizardPage.lbl.type"), SWT.READ_ONLY); //$NON-NLS-1$
	...
	protected void openFileDialog() {
		...
		lDialog.setText(RelationsMessages
				.getString("PrintOutWizardPage.lbl.dialog")); //$NON-NLS-1$
	...
	private Collection<Control> createPrintOutScope(final Composite inParent) {
		...
		lSelectionLabel.setLayoutData(new GridData(SWT.FILL, SWT.TOP, false,
				false));
	...
	private Button createLayoutCheckbox(final Composite inParent) {
		...
		lCheckboxLabel.setText(RelationsMessages
				.getString("PrintOutWizardPage.lbl.output")); //$NON-NLS-1$
	...
	private Collection<Control> createRadios(final Composite inParent,
			final Integer[] inIds, final String[] inLabels,
			final int inDefault, final SelectionListener inListener) {
		final Collection<Control> outRadios = new Vector<Control>();
