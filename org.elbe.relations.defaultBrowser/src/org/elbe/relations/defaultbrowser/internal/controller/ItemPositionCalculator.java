...
public class ItemPositionCalculator {
	 ...
	 */
	public ItemPositionCalculator(final long inWidth, final long inHeight, final long inRadius,
			final long inRemaining) {
		super();
		width = inWidth;
		height = inHeight;
		radius = inRadius;
	...
	private void init(final long inRadius, final long inRemaining) {
		...
		delta = Math.acos(Math.cos(offset) - (height * 1.0 / inRadius)) - offset;
		...
		if (inRemaining <= count) {
			...
			offset = (inRemaining < 3 ? Math.PI / 2 : (2 * Math.PI - (inRemaining - 2) * delta) / 4);
			count = inRemaining;
			hasMore = false;
		}
	}
	...
	public List<PrecisionPoint> getPositions() {
		final List<PrecisionPoint> outPositions = new ArrayList<PrecisionPoint>();
		for (int i = 0; i < count; i++) {
			if (i < getHalfOfCount()) {
				final double lAngle = offset + i * delta;
				outPositions.add(new PrecisionPoint(radius * Math.sin(lAngle), -1 * radius * Math.cos(lAngle)));
			} else {
				final double lAngle = Math.PI + offset + (i - getHalfOfCount()) * delta;
				outPositions.add(new PrecisionPoint(radius * Math.sin(lAngle), -1 * radius * Math.cos(lAngle)));
			}
		}
		return outPositions;
